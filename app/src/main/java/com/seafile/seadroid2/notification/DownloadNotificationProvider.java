...
public class DownloadNotificationProvider extends BaseNotificationProvider {
    ...
    protected String getProgressInfo(NotificationState state) {
        ...
        if (state != NotificationState.NOTIFICATION_STATE_PROGRESS) {
            return SeadroidApplication.getAppContext()
                    .getString(R.string.notification_download_completed);
        }

        int downloadingCount = 0;
        List<DownloadTaskInfo> infoList = txService.getAllDownloadTaskInfos();
        for (DownloadTaskInfo info : infoList) {
    ...
    public void notifyStarted() {
        ...
        PendingIntent dPendingIntent = PendingIntent.getActivity(
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
            txService.startForeground(NOTIFICATION_ID_DOWNLOAD, mNotifBuilder.build(), ServiceInfo.FOREGROUND_SERVICE_TYPE_DATA_SYNC);
        } else {
