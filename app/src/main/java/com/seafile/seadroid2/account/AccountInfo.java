...
public class AccountInfo {
    ...
    public static  AccountInfo fromJson(JSONObject accountInfo, String server) throws JSONException {
        ...
        info.contactEmail = accountInfo.optString("contact_email");
...
//    }
    ...
    public String getSpaceUsed(String from) {
        if (total < 0)
            return  "---";
        String strTotal = Utils.readableFileSize(total);
        long realUsage = usage > 0 ? usage : 0;
        return String.format("%.2f", 100 * (float)realUsage / (float)total) + " % " + from + " " + strTotal;
    }
