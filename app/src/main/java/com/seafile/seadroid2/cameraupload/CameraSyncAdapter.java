...
//        }
        ...
        try {
            ...
            UploadNotificationProvider uploadNotificationProvider = new UploadNotificationProvider(
                    txService.getUploadTaskManager(),
                    txService);
            txService.saveUploadNotifProvider(uploadNotificationProvider);
...
//                Utils.utilsLogInfo(true, "Skipping media " + file + " because it doesn't exist");
    ...
    private void uploadFile(DataManager dataManager, File file, String bucketName) throws SeafException {
        ...
        if (repo != null && repo.canLocalDecrypt()) {
            taskID = txService.addTaskToSourceQueBlock(Utils.TRANSFER_PHOTO_TAG, dataManager.getAccount(), targetRepoId, targetRepoName,
                    serverPath, file.getAbsolutePath(), false, false);
        }
        else {
            taskID = txService.addCameraUploadTask(dataManager.getAccount(), targetRepoId, targetRepoName,
                    serverPath, file.getAbsolutePath(), false, false);
        }
    ...
    private void showNotificationAuthError() {
        ...
        PendingIntent dPendingIntent = null;
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            dPendingIntent = PendingIntent.getActivity(getContext(),
