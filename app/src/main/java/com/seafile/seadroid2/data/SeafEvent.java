...
public class SeafEvent implements SeafItem {
    ...
    public static SeafEvent fromJson2(JSONObject obj, SeafRepo repo) {
        SeafEvent event = new SeafEvent();
        try {
            event.commit_id = obj.optString("commit_id");
            event.v_time = obj.optString("time");
            event.time = SystemSwitchUtils.parseISODateTime(obj.getString("time"));
            event.desc = obj.optString("description");
            event.name = obj.optString("name");
            event.obj_type ="";
            event.op_type = "";
            event.nick = "";
            if (repo != null) {
                ...
                event.repo_name = repo.name;
                event.repo_encrypted = repo.encrypted;

                event.author = obj.optString("email");
                event.author_name = obj.optString("name");
            } else {
                event.repo_id = obj.optString("repo_id");
                event.repo_name = obj.optString("repo_name");
                event.repo_encrypted = obj.optBoolean("repo_encrypted");
    ...
    public static JSONObject fromString(SeafEvent event) {
        JSONObject jsonObject = new JSONObject();
        try {
            jsonObject.put("commit_id", event.commit_id);
            jsonObject.put("time", event.v_time);
            jsonObject.put("description", event.desc);
            jsonObject.put("author", event.author);
            jsonObject.put("author_name", event.author_name);
            jsonObject.put("repo_name", event.repo_name);
            jsonObject.put("name", event.name);
            jsonObject.put("obj_type", event.obj_type);
            jsonObject.put("op_type", event.op_type);
            jsonObject.put("nick", event.nick);
            jsonObject.put("repo_id", event.repo_id);
            jsonObject.put("repo_name", event.repo_name);
            jsonObject.put("repo_encrypted", event.repo_encrypted);
            return jsonObject;
        } catch (Exception e) {
            Log.d(DEBUG_TAG, e.getMessage());
            return null;
    ...
    public static class EventTimeComparator implements Comparator<SeafEvent> {
        @Override
        public int compare(SeafEvent p1, SeafEvent p2) {
