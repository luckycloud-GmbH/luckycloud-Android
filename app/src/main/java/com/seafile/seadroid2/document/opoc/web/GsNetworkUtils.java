...
#########################################################*/
package com.seafile.seadroid2.document.opoc.web;
...
public class GsNetworkUtils {
    ...
    public static boolean downloadFile(final String url, final File out, final GsCallback.a1<Float> progressCallback) {
        try {
    ...
    public static boolean downloadFile(final URL url, final File outFile, final GsCallback.a1<Float> progressCallback) {
        return downloadFile(url, outFile, null, progressCallback);
    }

    public static boolean downloadFile(final URL url, final File outFile, HttpURLConnection connection, final GsCallback.a1<Float> progressCallback) {
    ...
    private static String performCall(final URL url, final String method, final String data, final HttpURLConnection existingConnection) {
        try {
            ...
            return GsFileUtils.readCloseTextStream(connection.getInputStream());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return "";
    ...
    public static void httpGetAsync(final String url, final GsCallback.a1<String> callback) {
        new Thread(() -> {
            try {
                String c = GsNetworkUtils.performCall(url, GET);
                callback.callback(c);
