...
public abstract class TextConverterBase {
    ...
    protected String putContentIntoTemplate(Context context, String content, boolean isExportInLightMode, File file, String onLoadJs, String head) {
        ...
        html = html
                ...
                .replace(TOKEN_COLOR_GREY_OF_THEME, darkTheme ? "#393939" : GsTextUtils.colorToHexString(ContextCompat.getColor(context, R.color.light_gray)))
                .replace(TOKEN_LINK_COLOR, GsTextUtils.colorToHexString(Color.parseColor("#388E3C")))
                .replace(TOKEN_ACCENT_COLOR, GsTextUtils.colorToHexString(ContextCompat.getColor(context, R.color.red)))
                .replace(TOKEN_TEXT_DIRECTION, "left")
    ...
    public boolean isFileOutOfThisFormat(final @NonNull File file) {
        final String name = file.getName().toLowerCase().trim();
