...
public class EditorActivity extends BaseActivity implements Toolbar.OnMenuItemClickListener {
    ...
    private void markdown() {
        ...
        String content = readToString(new File(path));
        ...
        try {
            ...
            e.printStackTrace();
            Utils.utilsLogInfo(true, "parseMarkdown\nError: " + e);
            Toast.makeText(this, getString(R.string.failed_parse_markdown), Toast.LENGTH_SHORT).show();
        }
    }

    private String sanitizeMarkdown(String input) {
        if (input == null) return "";
        // Fix malformed unordered list items like "*Text" -> "* Text"
        input = input.replaceAll("(?m)^([*-])(\\S)", "$1 $2");
        // Add any other sanitizations here if needed
        return input;
    }
