        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                for (AutoUpdateInfo info : infos) {
                    if (Utils.isTextMimeType(info.localPath)) {
                        removeAutoUpdateInfo(info.account, info.repoID, info.repoName, info.parentDir, info.localPath);
                        continue;
                    }
                    if (info.canLocalDecrypt()) {
                        DataManager dataManager = new DataManager(info.account);
                        SeafRepo repo = dataManager.getCachedRepoByID(info.repoID);
                        if (repo != null && repo.canLocalDecrypt())
                            txService.addTaskToUploadQueBlock(info.account, info.repoID, info.repoName,
