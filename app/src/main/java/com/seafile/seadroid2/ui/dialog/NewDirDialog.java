...
class NewDirTask extends TaskDialog.Task {
    ...
    public NewDirTask(String repoID, String parentDir,
                      String dirName, DataManager dataManager, String currentPath) {
        this.repoID = repoID;
    ...
    protected void runTask() {
        try {
            if (currentPath == null) {
                ...
                String dirPath = Utils.removeLastPathSeperator(currentPath) + "/" + dirName;
...
public class NewDirDialog extends TaskDialog {
    ...
    public void init(String repoID, String parentDir, Account account, String currentPath) {
        this.repoID = repoID;
        this.parentDir = parentDir;
...
//        dialog.setTitle(getResources().getString(R.string.create_new_dir));
        updateTitleText(getResources().getString(R.string.create_new_dir));
    ...
    protected NewDirTask prepareTask() {
        ...
        String dirName = dirNameText.getText().toString().trim();
        if (currentPath == null) {
            return new NewDirTask(repoID, parentDir, dirName, getDataManager(), null);
        } else {
            return new NewDirTask(null, null, dirName, null, currentPath);
        }
    }
}