...
class SetPasswordTask extends TaskDialog.Task {
    ...
    protected void runTask() {
        ...
        try {
            if (repo == null) {
                // repo not already cached -> get Repo from Server and write into Cache
                dataManager.getEncryptRepo(repoID);
                repo = dataManager.getCachedRepoEncryptByID(repoID);
            }
...
//        dialog.setTitle(repoName);
        updateTitleText(getString(R.string.password_from_library));
        dialog.getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_VISIBLE);
        showDoubleCardLayout();
        updateCancelText(getResources().getString(R.string.close));
        updateOkText(getResources().getString(R.string.open));
    }
